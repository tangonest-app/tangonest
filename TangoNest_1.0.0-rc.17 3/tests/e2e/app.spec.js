...
test("updated release starts at Login even when an older session was saved",async({page})=>{
  await installAuthMock(page,{startSignedIn:true,skipFreshLoginMarker:true});
  await page.goto("/");
  await expect(page.locator("#authScreen")).toBeVisible();
  await expect.poll(()=>page.evaluate(()=>window.tnAuthDiagnostics().authenticated)).toBe(false);
  await expect.poll(()=>page.evaluate(()=>localStorage.getItem("tangonest_e2e_auth_session"))).toBe(null);
});
