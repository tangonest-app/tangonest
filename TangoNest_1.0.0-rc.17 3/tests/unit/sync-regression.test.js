  ...
  return {
    ...
    rpc: async (name,args = {}) => {
      ...
      if(name === "tn_apply_account_clean_start_v1"){
        ...
        const migrationKey=`${userId}:account-clean-start-v1`;
        if(state.accountMigrations.has(migrationKey)){
          return {data:{
            ...
            migration:"account-clean-start-v1"
          },error:null};
        }
        ...
        return {data:{applied:true,words:0,lists:1,playlist_id:created.id,migration:"account-clean-start-v1"},error:null};
      }
...
function createContext({session, playlists = [], words = [], playlistStore = null, wordStore = null, migrationStore = null, failCloud = false, failTables = [], tableError = "", storage = {}, initialDb = null, signUpSession = undefined, signUpError = null, loginSession = undefined, loginError = null, sessionError = null, localQa = false, missingEnsureRpc = false, forceFreshLogin = false, forceAccountReset = false}){
  ...
  const state = {session,signUpSession,signUpError,loginSession,loginError,sessionError,playlists:playlistStore||[...playlists],words:wordStore||[...words],failCloud,failTables:new Set(failTables),tableError,channels:[],warnings:[],errors:[],learningEvents:new Set(),accountMigrations:migrationStore||new Set(),cleanStartCalls:0,missingEnsureRpc};
  const mockConsole = {
  ...
  if(!forceFreshLogin&&!localQa&&storageSeed[FRESH_LOGIN_RELEASE_KEY]===undefined)storageSeed[FRESH_LOGIN_RELEASE_KEY]="complete";
...
(async () => {
  ...
  const forcedLoginApp=await bootApp({
    session:{user:userA},
    forceFreshLogin:true,
    initialDb:staleLocalData,
    storage:{
  ...
  assert.strictEqual(forcedLoginApp.state.session,null,"the release opens signed out even when an old session exists");
  assert.strictEqual(forcedLoginApp.window.tnAuthDiagnostics().authenticated,false,"the first updated launch shows the login state");
  assert.strictEqual(forcedLoginApp.window.db.words.length,0,"the login reset removes stale local words");
  assert.strictEqual(forcedLoginApp.window.db.lists.length,1,"the login reset exposes one local default only");
  assert.strictEqual(forcedLoginApp.storage.getItem(FRESH_LOGIN_RELEASE_KEY),"complete","the login reset is recorded once per device");
  assert.strictEqual(forcedLoginApp.storage.getItem(PENDING_KEY),null,"obsolete pending writes cannot recreate deleted data");
