...
Run the root-level `SUPABASE_SCHEMA_CURRENT.sql` once in the Supabase SQL Editor before deploying rc.17. It is the single canonical, idempotent-oriented schema and migration for existing and new projects. For this clean-start release, the first authenticated launch intentionally resets that account to `0 Words / 1 My Words list`; the database records the migration so later launches never repeat the deletion.
