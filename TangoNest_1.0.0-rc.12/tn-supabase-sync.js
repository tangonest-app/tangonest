(function(){
  ...
  async function ensureDefaultPlaylist(userId=currentUser?.id){
    await requireUser();
    if(!userId || currentUser?.id!==userId)throw new Error("Session changed during data load.");
    const existing = await client.from("tn_playlists").select("*").eq("user_id",userId).order("created_at",{ascending:true});
    if(existing.error)throw existing.error;
    if(existing.data?.length)return existing.data[0];
    const inserted = await client.from("tn_playlists")
      .insert({user_id:userId,name:"New Playlist"})
      .select("*")
      .single();
      ...
      try{
        ...
        if(playlistsResult.error){
          ...
          if(!playlistRows.length){
            try{
              await ensureDefaultPlaylist(loadUserId);
              ...
              errors.push({scope:"playlists",error});
              playlistRows=null;
            }
          }
        }
        ...
        const words=wordRows===null ? (previous.words || []) : wordRows.map(row => toLocalWord(row,firstListId));
