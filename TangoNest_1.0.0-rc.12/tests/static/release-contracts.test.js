...
const authFixSql=read("SUPABASE_AUTH_FIX.sql");
...
assert.match(serviceWorker,/tangonest-shell-v1\.0\.0-rc\.12/);
...
assert.ok((style.match(/!important/g)||[]).length<1300,"Legacy !important count must not regress");
...
assert.match(sync,/function readAllPending\(\)/);
assert.match(sync,/otherUsers.*userId !== currentUser\?\.id/);
...
assert.doesNotMatch(authFixSql,/drop\s+table|truncate|delete\s+from/i,"Auth repair must never delete user data");
assert.match(authFixSql,/alter table public\.tn_words add column if not exists position integer/i);
assert.match(authFixSql,/grant select,insert,update,delete on table public\.tn_words to authenticated/i);
assert.match(authFixSql,/create policy tn_words_select_own[\s\S]+auth\.uid\(\)=user_id/i);
assert.match(authFixSql,/create policy tn_playlists_select_own[\s\S]+auth\.uid\(\)=user_id/i);
assert.match(authFixSql,/notify pgrst,'reload schema'/i);
...
assert.equal(version,"1.0.0-rc.12");
for(const asset of ["style.css","app.js","learning-engine.js","tn-supabase-sync.js","tn-library-management.js","tn-learning-flow.js","ui/learning-presentation.js","ui/runtime.js"]){
  assert.match(html,new RegExp(`${asset.replace(/[.]/g,"\\.")}\\?v=${version.replace(/[.]/g,"\\.")}`),`Versioned asset: ${asset}`);
}
