const fs=require("node:fs");
const path=require("node:path");
const assert=require("node:assert/strict");

const root=path.resolve(__dirname,"../..");
const read=file=>fs.readFileSync(path.join(root,file),"utf8");
const html=read("index.html");
const app=read("app.js");
const library=read("tn-library-management.js");
const flow=read("tn-learning-flow.js");
const style=read("style.css");
const focusStyle=read("ui/study-focus.css");
const engine=read("learning-engine.js");
const sync=read("tn-supabase-sync.js");

assert.match(html,/ui\/learning-presentation-rc19-forest1\.js/);
assert.match(html,/Today’s Session/);
assert.match(html,/Auto · smart timing/);
assert.match(html,/Advanced sync details/);
assert.match(html,/role="status" aria-live="polite"/);
assert.doesNotMatch(html,/id="quizNextDelay"/);
assert.doesNotMatch(html,/writes to Supabase|counts are read from Supabase/i);

assert.match(engine,/const CONFIG=Object\.freeze/);
assert.match(engine,/\bCONFIG,\n/);
assert.match(app,/learningPresentation\(\)/);
assert.match(app,/const LANGUAGE_OPTIONS=Object\.freeze/);
assert.match(app,/if\(mode==="auto"\)return ok\?1000:1600/);
assert.match(app,/\.choice\[data-shortcut=/);
assert.doesNotMatch(app,/__tnHeroLanguageTimer/);
assert.doesNotMatch(app,/run the Supabase setup first/i);
assert.doesNotMatch(flow,/tn-learning-dashboard/);
assert.doesNotMatch(library,/new MutationObserver/);
assert.match(library,/data-library-view="due"/);
assert.match(library,/tnFilterFavorite/);
assert.match(library,/tnFilterLearning/);
assert.match(library,/tnSortWords/);
assert.match(library,/data-rename-playlist/);
assert.match(library,/data-select-visible/);
assert.match(library,/data-delete-selected/);
assert.match(library,/data-load-all/);
assert.match(library,/tn-word-list-pos/);
assert.doesNotMatch(library,/class="tn-word-example"/);
assert.match(library,/document\.addEventListener\("change",event => \{/);
assert.doesNotMatch(html,/id="wordsBox"/);
assert.doesNotMatch(sync,/const logout = \$\("tn80LogoutBtn"\)/);
assert.doesNotMatch(sync,/Supabase SDK is still loading|currentUser \? "Supabase Auth"/);
assert.match(sync,/Online login is temporarily unavailable\. Check your connection and try again\./);
assert.match(sync,/allowPartial:false/);
assert.match(sync,/You are signed in, but cloud vocabulary is unavailable/);
assert.match(sync,/initializedUserId===session\.user\.id/);
assert.match(html,/id="togglePasswordButton"/);
assert.match(html,/id="forgotPasswordButton"/);
assert.match(html,/id="updatePasswordButton"/);
assert.match(html,/class="app-sidebar"/);
assert.match(html,/class="app-workspace"/);
assert.match(html,/ui\/study-focus-rc19-forest1\.css/);
assert.match(html,/class="home-focus"/);

assert.equal((style.match(/^:root\{/gm)||[]).length,1);
assert.match(style,/TangoNest Design System/);
assert.match(style,/@media\(prefers-reduced-motion:reduce\)/);
assert.match(style,/\.tn-learning-badge\.mastered/);
assert.match(style,/overflow-x:clip/);
assert.match(focusStyle,/TangoNest Study Focus theme/);
assert.match(focusStyle,/--focus-canvas:#f2ecdf/);
assert.match(focusStyle,/Forest Journal visual system/);
assert.match(focusStyle,/--forest-deep:#0e382a/);
assert.match(focusStyle,/grid-template-columns:var\(--focus-sidebar\) minmax\(0,1fr\)/);
assert.match(focusStyle,/\.mobile-add-button/);
assert.match(focusStyle,/grid-template-columns:minmax\(90px,1\.15fr\)/);
assert.match(focusStyle,/\.quiz-focus-active/);
assert.doesNotMatch(focusStyle,/linear-gradient|radial-gradient|backdrop-filter:blur/i);

console.log("UI_CONTRACTS_TEST_PASS");
