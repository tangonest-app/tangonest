const {test,expect,openTestApp,navigateToPage,selectLanguage}=require("./fixtures");
...
test("twelve real-user journeys remain direct and coherent",async({page})=>{
  ...
  await test.step("3. Library search and filters find a word without layout drift",async()=>{
    ...
    await expect(page.locator(".tn-word-row")).toHaveCount(11);
    const panel=page.locator(".tn-library-filter-panel");
    if(!await panel.evaluate(node=>node.open))await panel.locator("summary").click();
    await expect.poll(()=>panel.evaluate(node=>node.open)).toBe(true);
  ...
  await test.step("8. French to English uses the same language priority",async()=>{
    await navigateToPage(page,"create");
    await selectLanguage(page,"bulkFrontLang","fr-FR");
    await selectLanguage(page,"bulkBackLang","en-US");
  ...
  await test.step("9. English to Japanese preferences remain available",async()=>{
    ...
    await selectLanguage(page,"frontLang","en-US");
    await selectLanguage(page,"backLang","ja-JP");
  });
