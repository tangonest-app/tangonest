...
begin
  ...
  loop
    ...
    where user_id=v_user_id
      and lower(btrim(name))='my words'
    order by
      case
        when is_default and lower(btrim(name))='my words' then 0
        else 1
      end,
      created_at,
      id
    ...
    update public.tn_playlists
    set name='My Words',is_default=true
    where id=v_default_id
      and user_id=v_user_id
      and (name is distinct from 'My Words' or is_default is distinct from true);
  end loop;
end $$;
...
begin
  ...
  if v_target.is_default then
    raise exception 'The default My Words playlist cannot be deleted';
  end if;
...
begin
  ...
  loop
    ...
    if lower(v_list_name)='my words' and v_default_assigned then
      ...
      insert into public.tn_playlists(user_id,name,is_default)
      values (v_user_id,case when v_is_default then 'My Words' else v_list_name end,v_is_default)
