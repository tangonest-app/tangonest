-- TangoNest v1.0.0-rc19 canonical production schema and one-time isolation reset
-- Run in Supabase SQL Editor.
--
-- IMPORTANT:
-- This authorized migration deletes TangoNest learning data once, preserves
-- auth.users, and recreates exactly one independent My Words row per account.
...
alter table public.tn_playlists add column if not exists is_default boolean not null default false;

-- Existing installations may predate the composite key. It must exist before
-- the matching two-column foreign key is recreated below.
do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.tn_playlists'::regclass
      and conname='tn_playlists_id_user_unique'
  ) then
    alter table public.tn_playlists
      add constraint tn_playlists_id_user_unique unique (id,user_id);
  end if;
end $$;

-- Words may remain unfiled. "All Words" is a virtual client view, never a row.
do $$
declare
  v_constraint record;
begin
...
create table if not exists public.tn_account_migrations (
  user_id uuid not null references auth.users(id) on delete cascade,
...
create table if not exists public.tn_system_migrations (
...
-- Repair every existing account before the one-default index is enforced.
-- Only empty rows with exact historical generated names and generation/repair
-- timestamps tied to My Words are removed.
do $$
declare
  v_user_id uuid;
  v_default_id uuid;
begin
  ...
  loop
    ...
    where user_id=v_user_id
      and (is_default or lower(btrim(name))='my words')
    order by
      case
        when is_default then 0
        when lower(btrim(name))='my words' then 1
        else 2
      end,
      created_at,
      id
    ...
    if v_default_id is null then
      ...
      values (v_user_id,'My Words',false)
      returning id into v_default_id;
    end if;

    -- My Words is a reserved system playlist. Merge every historical duplicate
    -- into the canonical row before deleting it so vocabulary is never lost.
    update public.tn_words w
    set playlist_id=v_default_id,
        content_updated_at=now(),
        updated_at=now()
    where w.user_id=v_user_id
      and w.playlist_id in (
        select p.id
        from public.tn_playlists p
        where p.user_id=v_user_id
          and p.id<>v_default_id
          and lower(btrim(p.name))='my words'
      );

    delete from public.tn_playlists p
    where p.user_id=v_user_id
      and p.id<>v_default_id
      and lower(btrim(p.name))='my words';
    ...
    where p.user_id=v_user_id
      and p.id<>v_default_id
      and lower(btrim(p.name)) in ('my words','new playlist','starter','default','chinese')
      and exists (
        select 1
        from public.tn_playlists d
        where d.id=v_default_id
          and d.user_id=v_user_id
          and (
            (lower(btrim(p.name))<>'chinese' and abs(extract(epoch from (p.updated_at-p.created_at)))<=5)
            or abs(extract(epoch from (p.updated_at-d.created_at)))<=5
            or abs(extract(epoch from (p.updated_at-d.updated_at)))<=5
          )
      )
      and not exists (
        select 1 from public.tn_words w
        where w.user_id=p.user_id and w.playlist_id=p.id
      );
...
create unique index if not exists tn_playlists_one_default_per_user_idx
  on public.tn_playlists(user_id)
  where is_default;

create unique index if not exists tn_playlists_one_my_words_per_user_idx
  on public.tn_playlists(user_id,(lower(btrim(name))))
  where lower(btrim(name))='my words';
...
alter table public.tn_account_migrations enable row level security;
alter table public.tn_system_migrations enable row level security;

alter table public.tn_playlists force row level security;
alter table public.tn_words force row level security;
alter table public.tn_learning_events force row level security;
alter table public.tn_account_migrations force row level security;
alter table public.tn_system_migrations force row level security;
...
declare
  v_user_id uuid := auth.uid();
  v_migration_key constant text := 'account-clean-start-v1';
  v_playlist_id uuid;
...
begin
  ...
  if exists (
    select 1 from public.tn_account_migrations
    where user_id=v_user_id and migration_key=v_migration_key
  ) then
  ...
  insert into public.tn_playlists(user_id,name,is_default)
  values (v_user_id,'My Words',true)
...
begin
  ...
  if v_target.is_default then
    raise exception 'The default playlist cannot be deleted';
  end if;
...
begin
  ...
  loop
    ...
    v_list_name := coalesce(nullif(btrim(v_list->>'name'),''),'Imported Playlist');
    v_is_default := not v_default_assigned and (
      lower(coalesce(v_list->>'isDefault','false')) in ('true','1','yes','on')
      or lower(v_list_name)='my words'
    );
    if lower(v_list_name)='my words' and v_default_assigned then
      v_new_list_id := v_default_id;
    else
      insert into public.tn_playlists(user_id,name,is_default)
      values (v_user_id,v_list_name,v_is_default)
      returning id into v_new_list_id;

      if v_is_default then
        v_default_id := v_new_list_id;
      end if;
      v_default_assigned := v_default_assigned or v_is_default;
  ...
  if not v_default_assigned then
    insert into public.tn_playlists(user_id,name,is_default)
    values (v_user_id,'My Words',true)
    returning id into v_new_list_id;
    v_default_id := v_new_list_id;
...
create or replace function public.tn_clean_example_fields()
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  for v_row in
    select id,memo,pronunciation
    from public.tn_words
    where user_id = v_user_id and nullif(btrim(memo),'') is not null
  loop
    v_memo := btrim(v_row.memo);
    v_pronunciation := nullif(btrim(v_row.pronunciation),'');
    v_example := v_memo;

    if position('|' in v_memo) > 1 then
      v_before := btrim(split_part(v_memo,'|',1));
      v_after := btrim(substring(v_memo from position('|' in v_memo) + 1));
      if char_length(v_before) between 1 and 80
        and v_before ~ '^[[:alpha:][:digit:]/''’ -]+$'
        and v_after <> ''
        and v_after ~ '[。！？!?\.]' then
        v_pronunciation := coalesce(v_pronunciation,v_before);
        v_example := v_after;
      end if;
    end if;

    if v_example = v_memo then
      v_suffix := substring(v_memo from '(/[^/\r\n]{1,80}/|[[:alpha:]][[:alpha:][:digit:]''’ -]{0,79})[[:space:]]*$');
      if v_suffix is not null then
        v_after := btrim(regexp_replace(v_memo,'[[:space:]]+(/[^/\r\n]{1,80}/|[[:alpha:]][[:alpha:][:digit:]''’ -]{0,79})[[:space:]]*$','','g'));
        if v_after ~ '[。！？!?\.]$'
          and (left(v_suffix,1) = '/' or v_after ~ '[一-龯ぁ-んァ-ン가-힣]') then
          v_pronunciation := coalesce(v_pronunciation,btrim(v_suffix));
          v_example := v_after;
        end if;
      end if;
    end if;

    if v_example <> v_memo or coalesce(v_pronunciation,'') <> coalesce(v_row.pronunciation,'') then
      update public.tn_words
      set memo = nullif(v_example,''),
          pronunciation = v_pronunciation,
          content_updated_at = now(),
          updated_at = now()
...
-- Authorized rc19 one-time clean start. This never deletes auth.users.
-- The system marker makes the destructive block safe to rerun.
do $$
declare
  v_migration_key constant text := 'rc19-account-isolation-production-reset-v1';
begin
  perform pg_advisory_xact_lock(hashtextextended(v_migration_key,0));
  if not exists (
    select 1 from public.tn_system_migrations where migration_key=v_migration_key
  ) then
    delete from public.tn_learning_events;
    delete from public.tn_words;
    delete from public.tn_playlists;
    delete from public.tn_account_migrations;

    alter table public.tn_playlists alter column user_id set not null;
    alter table public.tn_words alter column user_id set not null;
    alter table public.tn_learning_events alter column user_id set not null;
    alter table public.tn_account_migrations alter column user_id set not null;

    insert into public.tn_playlists(user_id,name,is_default)
    select id,'My Words',true from auth.users;

    insert into public.tn_account_migrations(user_id,migration_key)
    select id,'account-isolation-reset-v2' from auth.users
    on conflict (user_id,migration_key) do nothing;

    insert into public.tn_system_migrations(migration_key)
    values (v_migration_key);
  end if;
end $$;

revoke all on public.tn_playlists, public.tn_words, public.tn_learning_events, public.tn_account_migrations, public.tn_system_migrations from anon, authenticated;
...
revoke execute on function public.tn_clean_example_fields() from public, anon;
...
grant execute on function public.tn_clean_example_fields() to authenticated;
