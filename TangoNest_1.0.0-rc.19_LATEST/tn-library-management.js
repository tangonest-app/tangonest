(function(){
  ...
  function wordsTools(){
    ...
    const state = filterState();
    const filtersOpen=(window.matchMedia?.("(min-width: 901px)")?.matches??true)||filterPanelOpen;
        ...
        ${words.map(word => {
          ...
          <article class="tn82-word-card tn-word-row" data-word-id="${esc(word.id)}">
            ...
            <div class="tn82-word-meta">
              <span class="tn-word-playlist" title="${esc(listName(word.listId,data))}">${esc(listName(word.listId,data))}</span>
              <span class="tn-learning-badge ${esc(learning.tone)}">${esc(learning.label)}</span>
              <span class="tn-review-label">${esc(next)}</span>
              <span class="tn-word-pos">${word.pos ? esc(word.pos) : "—"}</span>
              <div class="tn-word-row-actions">
  ...
  function bindLibraryUi(){
    ...
    if(panel&&!panel.__tnToggleBound){
      panel.addEventListener("toggle",()=>{
        if(window.matchMedia?.("(max-width: 900px)")?.matches)filterPanelOpen=panel.open;
      });
