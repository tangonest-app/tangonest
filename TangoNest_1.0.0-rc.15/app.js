...
function tnNormalizeDataShape(value,reason){
  ...
  data.lists=Array.isArray(data.lists)?data.lists.filter(list=>list&&!["starter","local-starter"].includes(String(list.id||""))):[];
  ...
  data.words=data.words.map(word=>{
    const normalized={
      ...
      listId:validListIds.has(String(word.listId||""))?String(word.listId):"",
...
function tnIsDefaultList(list){
  ...
  return id==="starter" || id==="local-starter";
}
...
function renderSelect(id){
  ...
  db.lists.forEach(list=>{const option=document.createElement("option");option.value=list.id;option.textContent=list.name;el.appendChild(option)});
  if([...el.options].some(option=>option.value===previous))el.value=previous;
  else if(allWords)el.value="all";
  else if(unfiled)el.value="";
  const renameUnavailable=id==="renameListSelect"&&!db.lists.length;
