  ...
  return {
    ...
    rpc: async (name,args = {}) => {
      ...
      if(name === "tn_delete_all_account_data"){
        ...
        return {data:{words:0,lists:0},error:null};
      }
...
(async () => {
  ...
  assert.strictEqual(app.window.db.lists.length,0,"new account starts with no playlists");
  ...
  assert.strictEqual(app.elements.get("tn80CloudLists").textContent,"0","cloud list count is actual cloud count");
  ...
  assert.strictEqual(unfiledApp.state.playlists.length,0,"adding a word never invents a playlist");
  ...
  assert.strictEqual(app.state.playlists.length,0,"deleting the only playlist leaves zero playlists");
  ...
  assert.strictEqual(partialDataApp.window.db.lists.length,0,"zero playlist data still loads when words permission is broken");
  ...
  assert.strictEqual(importApp.state.playlists.length,0,"delete all account data leaves a true empty account");
  ...
  assert.strictEqual(loginApp.state.playlists.length,0,"login and SIGNED_IN listener create no default playlists");
