(function(){
  ...
  function ensureData(){
    ...
    data.lists=data.lists.filter(list=>!["starter","local-starter"].includes(String(list?.id||"")));
    const validListIds=new Set(data.lists.map(list=>list.id));
    data.words.forEach(word => {
      if(!validListIds.has(String(word.listId||"")))word.listId = "";
    });
    return data;
  }

  function listName(id){
    return ensureData().lists.find(list => list.id === id)?.name || "Unfiled";
  }
  ...
  function renderSelectsSafe(){
    ...
    ["addList","bulkList","studyList","quizList","audioList","renameListSelect","editList"].forEach(id => {
      ...
      el.innerHTML = virtual+data.lists.map(list => `<option value="${esc(list.id)}">${esc(list.name || "Untitled Playlist")}</option>`).join("");
      if([...el.options].some(option => option.value === current))el.value = current;
      else if(allWords)el.value="all";
      else if(unfiled)el.value="";
      const renameUnavailable=id==="renameListSelect"&&!data.lists.length;
