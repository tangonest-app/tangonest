-- TangoNest: non-destructive Supabase Auth and Learning Engine migration
...
create or replace function public.tn_delete_playlist_with_fallback(p_playlist_id uuid)
returns jsonb
language plpgsql
set search_path = public
as $$
...
begin
  ...
  if v_fallback_id is null then
    ...
    values (v_user_id,'New Playlist')
    returning id into v_fallback_id;
  end if;

  update public.tn_words
  set playlist_id = v_fallback_id,
      updated_at = now()
  where user_id = v_user_id
  ...
  return jsonb_build_object('deleted',p_playlist_id,'fallback',v_fallback_id,'moved_words',v_moved);
end;
$$;
...
begin
  ...
  loop
    ...
    v_fallback_id := coalesce(v_fallback_id, v_new_list_id);
  ...
  loop
    ...
    v_new_list_id := coalesce(nullif(v_list_map->>v_old_list_id,'')::uuid, v_fallback_id);
...
revoke execute on function public.tn_delete_playlist_with_fallback(uuid) from public, anon;
...
grant execute on function public.tn_delete_playlist_with_fallback(uuid) to authenticated;
