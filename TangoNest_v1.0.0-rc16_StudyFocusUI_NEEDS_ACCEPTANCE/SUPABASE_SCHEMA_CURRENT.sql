-- TangoNest v1.0.0-rc16 canonical production schema
...
begin
  ...
  loop
    ...
    v_is_default := not v_default_assigned and lower(coalesce(v_list->>'isDefault','false')) in ('true','1','yes','on');
    insert into public.tn_playlists(user_id,name,is_default)
    values (v_user_id, coalesce(nullif(btrim(v_list->>'name'),''),'Imported Playlist'),v_is_default)
