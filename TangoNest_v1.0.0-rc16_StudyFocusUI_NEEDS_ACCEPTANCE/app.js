...
function tnLocalDefaultPlaylist(at=tnNowIso()){
  return {id:TN_LOCAL_DEFAULT_PLAYLIST_ID,name:TN_DEFAULT_PLAYLIST_NAME,isDefault:true,createdAt:at,updatedAt:at};
}
...
function tnNormalizeDataShape(value,reason){
  ...
  const hadLegacyDefault=Array.isArray(data.lists)&&data.lists.some(list=>legacyDefaultIds.has(String(list?.id||"")));
  data.lists=Array.isArray(data.lists)?data.lists.filter(list=>list&&!legacyDefaultIds.has(String(list.id||""))):[];
  ...
  if(!data.lists.length||(hadLegacyDefault&&!data.lists.some(list=>list.isDefault)))data.lists.unshift(tnLocalDefaultPlaylist(at));
  let defaultSeen=false;
  data.lists.forEach(list=>{
    if(!list.isDefault)return;
    if(defaultSeen)list.isDefault=false;
