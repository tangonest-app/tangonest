(function(){
  ...
  function ensureData(){
    ...
    const hadLegacy=data.lists.some(list=>legacyIds.has(String(list?.id||"")));
    data.lists=data.lists.filter(list=>!legacyIds.has(String(list?.id||"")));
    if(!data.lists.length||hadLegacy&&!data.lists.some(isDefaultList)){
