(function(){
  ...
  const FRESH_LOGIN_RELEASE_KEY = "tangonest_fresh_login_rc17_pdca6";
  const ACCOUNT_CLEAN_START_PREFIX = "tangonest_account_clean_start_v1:";
  ...
  function hasMeaningfulLocalData(data){
    if(data?.words?.length)return true;
    return Array.isArray(data?.lists) && data.lists.some(list => {
      if(!list)return false;
      if(list.isDefault || list.is_default)return false;
      return String(list.id || "") !== LOCAL_DEFAULT_PLAYLIST_ID;
    });
  }
  ...
  function writeCache(next){
    ...
    try{
      ...
      if(typeof window.tnWriteData === "function")window.tnWriteData(safe);
        ...
        if(Array.isArray(safe.words)&&safe.words.length&&serialized.length<=1500000)localStorage.setItem(SHADOW_KEY,serialized);
        else localStorage.removeItem(SHADOW_KEY);
      }
      if(currentUser?.id){
        localStorage.setItem(CACHE_USER_KEY,currentUser.id);
        if(serialized.length<=1500000){
          localStorage.setItem(ACCOUNT_DATA_PREFIX+currentUser.id,serialized);
          if(Array.isArray(safe.words)&&safe.words.length)localStorage.setItem(ACCOUNT_SHADOW_PREFIX+currentUser.id,serialized);
          else localStorage.removeItem(ACCOUNT_SHADOW_PREFIX+currentUser.id);
        }else{
          localStorage.removeItem(ACCOUNT_DATA_PREFIX+currentUser.id);
          localStorage.removeItem(ACCOUNT_SHADOW_PREFIX+currentUser.id);
        }
  ...
  function clearAllLocalAppData(){
    try{
      ...
      keys.filter(Boolean).forEach(key => {
        if(
          key===DATA_KEY||key===SHADOW_KEY||key===PAGE_KEY||key===CACHE_USER_KEY||
          key===UNASSIGNED_BACKUP_KEY||key===PENDING_KEY||key===RECENT_PLAYLIST_KEY||
          key.startsWith(ACCOUNT_DATA_PREFIX)||key.startsWith(ACCOUNT_SHADOW_PREFIX)||
          key.startsWith(ACCOUNT_CLEAN_START_PREFIX)||/^sb-.*-auth-token$/i.test(key)
  ...
  async function requireFreshLoginForRelease(){
    if(LOCAL_QA_MODE)return false;
    try{
      if(localStorage.getItem(FRESH_LOGIN_RELEASE_KEY)==="complete")return false;
    }catch(e){}
    ...
    try{
      ...
      // The local auth token was already removed above. A network failure must
      // not restore the obsolete session or bypass this one-time login gate.
      console.warn("Local release sign-out completed without server confirmation",error);
    }
    currentSession=null;
    currentUser=null;
    resetDefaultPlaylistGuard();
    try{localStorage.setItem(FRESH_LOGIN_RELEASE_KEY,"complete");}catch(e){}
    adoptDb(emptyData("release-clean-login"));
    renderAll();
    showAuth("Log in to start TangoNest with 0 Words and 1 List.");
    return true;
  }
  ...
  async function applyAccountCleanStartOnce(){
    ...
    if(accountCleanStartPromise&&accountCleanStartUserId===userId)return accountCleanStartPromise;
    ...
    const task=(async()=>{
      ...
      const result=await client.rpc("tn_apply_account_clean_start_v1");
      if(result.error){
      ...
      if(currentUser?.id!==userId)throw new Error("Session changed during account initialization.");

      clearUserCache({discardPendingUserId:userId,purgeScopedUserId:userId});
      try{
        localStorage.removeItem(RECENT_PLAYLIST_KEY);
        localStorage.setItem(localMarker,"complete");
      }catch(e){}
      resetDefaultPlaylistGuard();
      adoptDb(emptyData("account-clean-start-v1"));
      writeCache(getDb());
      renderAll();
  ...
  function adoptAccountCache(userId,ownerBefore,reason){
    ...
    if(!ownerBefore){
      ...
      if(hasMeaningfulLocalData(unassigned) && !localStorage.getItem(UNASSIGNED_BACKUP_KEY)){
  ...
  async function afterSession(session,mode){
    ...
    try{
      ...
      setStartupState("SYNC_ERROR");
      updateCloudUi("Error");
      const message=userError(error,"Account initialization failed. Run SUPABASE_SCHEMA_CURRENT.sql once, then log in again.");
      showAuth(message);
    ...
    if(switchingUser || ownerMismatch)resetDefaultPlaylistGuard();
