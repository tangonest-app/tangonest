...
assert.match(sql,/tn_apply_account_isolation_reset_v2[\s\S]+pg_advisory_xact_lock[\s\S]+delete from public\.tn_learning_events[\s\S]+delete from public\.tn_words[\s\S]+delete from public\.tn_playlists[\s\S]+values \(v_user_id,'My Words',true\)[\s\S]+tn_account_migrations/i,"One locked account transaction creates the requested Word 0 / List 1 state");
assert.match(client,/persistSession:true[\s\S]+autoRefreshToken:true/,"Standard Supabase session persistence remains enabled");
assert.match(client,/ACCOUNT_CLEAN_START_PREFIX[\s\S]+tn_apply_account_isolation_reset_v2/,"Isolation reset runs before the first cloud hydration and is marked per account");
assert.match(client,/tangonest:account:v2:/,"Every user-owned browser cache uses the rc19 account namespace");
assert.match(sql,/rc19-account-isolation-production-reset-v1[\s\S]+delete from public\.tn_learning_events;[\s\S]+delete from public\.tn_words;[\s\S]+delete from public\.tn_playlists;[\s\S]+select id,'My Words',true from auth\.users/i,"Production reset clears learning data once and recreates one list per Auth user");
assert.doesNotMatch(sql,/delete\s+from\s+auth\.users/i,"Production reset preserves Auth users");
assert.match(sql,/alter table public\.tn_words force row level security/i,"User words enforce RLS even for table-owner execution paths");
...
assert.deepEqual(rootSql.sort(),["SUPABASE_SCHEMA_CURRENT.sql","TANGONEST_RC19_PRODUCTION_MIGRATION.sql"].sort(),"The root exposes only the canonical schema and the one-run rc19 migration");
