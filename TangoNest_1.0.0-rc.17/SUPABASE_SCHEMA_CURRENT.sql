...
-- Only empty, untouched rows with exact historical generated names are removed.
do $$
declare
  v_user_id uuid;
  v_default_id uuid;
begin
  ...
  loop
    ...
    where p.user_id=v_user_id
      ...
      and lower(btrim(p.name)) in ('my words','new playlist','starter','default')
      and abs(extract(epoch from (p.updated_at-p.created_at)))<=5
      and not exists (
